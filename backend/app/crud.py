...
def create_audio(db: Session, audio: schema.Audio, agent_id: int):
    db_audio = models.Audio(audio_path=audio.audio_path, filename = audio.filename,size=audio.size, duration=audio.duration, transcript=audio.transcript, timestamp=audio.timestamp, positivity_score=audio.positivity_score,
