...
def create_audio(db: Session, audio: schema.Audio, agent_id: int):
    db_audio = models.Audio(audio_path=audio.audio_path, transcript=audio.transcript, timestamp=audio.timestamp, size=audio.size, duration=audio.length, positivity_score=audio.positivity_score,
