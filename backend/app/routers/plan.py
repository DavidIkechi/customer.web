...
def convert_plan(plan_data: dict, user:models.User, db: Session):
    ...
    if old_plan != "Free":
        initial_plan = db.query(models.ProductPlan).filter(models.ProductPlan.name == old_plan).first()
        print(initial_plan)
        ...
        upload_time = int(cash_amount / upgrade_plan_price)
        ...
        # raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You need to Subscribe for a plan")
        return RedirectResponse(url="https://heed.cx/pricing", status_code=status.HTTP_303_SEE_OTHER)
        return {"message": "You need to subscribe for a plan"}
